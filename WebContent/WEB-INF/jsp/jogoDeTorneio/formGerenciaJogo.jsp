...
<title>${empresa} | Gerenciamento de jogos</title>
...
<div class="body_wrapper">
  <div class="center">
		...
		<c:import url="../comum/errors.jsp"/>
			...
			<div class="row">
				...
				<c:forEach items="${niveisDoTorneio}" var="nivelDoTorneio">								
					...
					<a href="${linkTo[TorneioController].detalhaTorneio(torneio.id)}" class="btn btn-default col-sm-4">Voltar</a>
				</div>
			</div>
		<c:if test="${tenistasInscritos!=null}">
		
		<form action="${linkTo[JogoDeTorneioController].insereJogoDeTorneio}" method="POST">
			...
			<div class="row">
				<div class="form-group col-sm-12">
					...
					<option value="0">Escolha um tenista ou informe jogo dependente</option>
					  <c:forEach items="${tenistasInscritos}" var="tenista">		
			...
			<label class="checkbox col-sm-12">
				<input type="checkbox" id="enviaEmail" name="enviaEmail"/>Enviar notificação de e-mail para os ${esportista}s (os que tenham jogo com dependência não recebem)</label>
						...
						<button class="btn btn-success col-sm-5">Inserir Jogo</button>
	...
	<br>
	<h4>Jogos registrados de ${nivel.descricao} no(a) ${torneio.nome}</h4><br/>
			...
			<td>${jogo.tenistaUm.nomeRanking} ${jogo.obsTenistaUm}</td>
			<td>${jogo.tenistaDois.nomeRanking} ${jogo.obsTenistaDois}</td>
			...
			<tr>
			<td colspan="5"><a href="${linkTo[JogoDeTorneioController].formEditaJogoDeTorneio(jogo.id)}" class="btn btn-primary">Editar Jogo</a>
