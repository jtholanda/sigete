...
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
 <title>Portal do Tênis Paraibano | Edição dos dados do tenista</title>
        ...
        <div class="single_left_coloum_wrapper">
			...
			<input type="hidden" id="id" name="tenista.id" class="form-control" value="${tenista.id}"/>
						...
						<div class="form-group col-sm-3">
							...
							<select name="tenista.raquete" id="raquete" class="form-control">
							 <option value="">Escolha uma raquete</option>
							  <c:if test="${tenista.raquete eq('Babolat')}">
								  <option value="Babolat" selected="selected">Babolat</option>
							  </c:if>
							  <c:if test="${tenista.raquete ne('Babolat')}">
								  <option value="Babolat">Babolat</option>
							  </c:if>
							  <c:if test="${tenista.raquete eq('Dunlop')}">
								  <option value="Dunlop" selected="selected">Dunlop</option>
							  </c:if>
							  <c:if test="${tenista.raquete ne('Dunlop')}">
					...
					<div class = "row">
						...
						<c:if test="${administrador}">
							<label for="mudarNivel">Nível Técnico * <br/><input type="checkbox" id="mudarNivel" name="mudarNivel" />
						...
						<select name="tenista.nivelTecnicoPrincipal.id" id="nivelTecnico" class="form-control" disabled="disabled">
							...
							<small>O seu nível técnico só poderá ser alterado através de solicitação a comissão da FPbT.</small>
				...
				<div class="btn-toolbar" align="center">
					<button type="submit" class="form-group col-sm-12 btn btn-success pull-center">Salvar dados</button>
				</div>
			</form>
		</div>
